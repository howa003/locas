...
def plot_gif_frame(time_step,
                   mesh_space: MeshSpace, results: Results, mesh_time: MeshTime) -> None:
    # Hard inputs
    ...
    y_min = np.amin(np.amin(strains_theoretical), np.amin(strains_fixed), np.amin(strains_clamped), np.amin(strains_free))
    y_max = np.amax(np.amax(strains_theoretical), np.amax(strains_fixed), np.amax(strains_clamped), np.amax(strains_free))
    y_lim_min = round_down(y_min, 4)
    y_lim_max = round_up(y_max, 4)
    ax2.plot(x_axis, strains_theoretical, 'k--',
             label='Theor. therm. strain: <' + num_to_str_1_dec(np.amin(strains_theoretical)) + ',' + num_to_str_1_dec(np.amax(strains_theoretical)) + '>',
             clip_on=clip_state)
    ax2.plot(x_axis, strains_internal_pressure, linestyle=(0, (3, 3, 1, 3)), color='k',
             label='Internal pressure strain: <' + num_to_str_1_dec(np.amin(strains_internal_pressure)) + ',' + num_to_str_1_dec(np.amax(strains_internal_pressure)) + '>',
             clip_on=clip_state)
    ax2.plot(x_axis, strains_prestressing, linestyle=(0, (3, 3, 1, 3, 1, 3)), color='k',
             label='Prestressing strain: <' + num_to_str_1_dec(np.amin(strains_prestressing)) + ',' + num_to_str_1_dec(np.amax(strains_prestressing)) + '>',
             clip_on=clip_state)
    ax2.plot(x_axis, strains_fixed, 'r-',
             label='Real strain (fixed BC): <' + num_to_str_1_dec(np.amin(strains_fixed)) + ',' + num_to_str_1_dec(np.amax(strains_fixed)) + '>',
             clip_on=clip_state)
    ax2.plot(x_axis, strains_clamped, 'm-',
             label='Real strain (clamped BC): <' + num_to_str_1_dec(np.amin(strains_clamped)) + ',' + num_to_str_1_dec(np.amax(strains_clamped)) + '>',
             clip_on=clip_state)
    ax2.plot(x_axis, strains_free, 'b-',
             label='Real strain (free BC): <' + num_to_str_1_dec(np.amin(strains_free)) + ',' + num_to_str_1_dec(np.amax(strains_free)) + '>',
             clip_on=clip_state)
    ...
    y_min = np.amin(np.amin(stresses_fixed), np.amin(stresses_clamped), np.amin(stresses_free))
    y_max = np.amax(np.amax(stresses_fixed), np.amax(stresses_clamped), np.amax(stresses_free))
    ...
    y_min = np.amin(np.amin(stresses_fixed_concrete), np.amin(stresses_clamped_concrete), np.amin(stresses_free_concrete))
    y_max = np.amax(np.amax(stresses_fixed_concrete), np.amax(stresses_clamped_concrete), np.amax(stresses_free_concrete))
    y_lim_min = round_down(y_min, 1)
    y_lim_max = round_up(y_max, 1)
    ax4.plot(x_axis, stresses_fixed_concrete, 'r-',
             ...
             clip_on=clip_state)
    ax4.plot(x_axis, stresses_clamped_concrete, 'm-',
             ...
             clip_on=clip_state)
    ax4.plot(x_axis, stresses_free_concrete, 'b-',
...
def create_gif_animation(mesh_space: MeshSpace, mesh_time: MeshTime, results: Results) -> None:
    ...
    for time_step in range(int(mesh_time.time_steps_count) + 1):
        ...
        frame = plot_gif_frame(time_step, mesh_space, results, mesh_time)
        frames.append(frame)
