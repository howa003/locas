...
def subplot_stress_distr(ax, subplot_title, x_axis, y_axis, x_min, x_max, y_min, y_max, mesh_space, structure) -> None:
    """
    This function plots a stress distribution inside the wall.
    ...
    legend_label = 'Stress in concrete: <' + num_to_str_1_dec(np.amin(y_axis_concrete)) + ',' + num_to_str_1_dec(
        np.amax(y_axis_concrete)) + '> MPa'
    ...
    if structure.has_inner_steel:
        legend_label = 'Stress in inner steel: <' + num_to_str_1_dec(np.amin(y_axis_steel_inner)) + ',' + num_to_str_1_dec(
            np.amax(y_axis_steel_inner)) + '> MPa'
        ax.plot(x_axis_steel_inner, y_axis_steel_inner, 'r-', label=legend_label, clip_on=False)
        ax.plot(np.array([x_axis_steel_inner[-1], x_axis_concrete[0]]), np.array([y_axis_steel_inner[-1], y_axis_concrete[0]]),
                linestyle=(0, (1, 3)), color=[0.5, 0.5, 0.5])

    # Plot the stress distribution for the outer steel liner
    if structure.has_outer_steel:
        legend_label = 'Stress in outer steel: <' + num_to_str_1_dec(np.amin(y_axis_steel_outer)) + ',' + num_to_str_1_dec(
            np.amax(y_axis_steel_outer)) + '> MPa'
        ax.plot(x_axis_steel_outer, y_axis_steel_outer, 'g-', label=legend_label, clip_on=False)
        ax.plot(np.array([x_axis_concrete[-1], x_axis_steel_outer[0]]), np.array([y_axis_concrete[-1], y_axis_steel_outer[0]]),
                linestyle=(0, (1, 3)), color=[0.5, 0.5, 0.5])
...
def plot_stress_distributions(folder_path, file_name, figure_title,
                              step_fixed: int, step_clamped: int, step_free: int,
                              structure: Structure, results: Results, mesh_space: MeshSpace, mesh_time: MeshTime) -> None:
    """
    ...
    try:
        ...
        subplot_time = float(mesh_time.time_axis[step_fixed])
        ...
        subplot_time = float(mesh_time.time_axis[step_clamped])
        ...
        subplot_time = float(mesh_time.time_axis[step_free])
