...
def calc_operating_temperatures(structure: Structure, mesh_space: MeshSpace) -> npt.NDArray[np.float64]:
    temp_air_int: float = structure.temp_air_int
    temp_air_ext: float = structure.temp_air_ext
    temp_surf_int: float = temp_air_int
    temp_surf_ext: float = temp_air_ext
    while True:
        temperature_distribution = calc_steady_state_heat_transfer(structure, mesh_space, temp_surf_int, temp_surf_ext)
        temp_surf_int_new = float(temperature_distribution[0])
        temp_surf_ext_new = float(temperature_distribution[-1])
...
def calc_steady_state_heat_transfer(structure: Structure, mesh_space: MeshSpace, temp_surf_int: float, temp_surf_ext: float) -> npt.NDArray[np.float64]:
    ...
    resistance_air_in = calc_surface_resistance(structure, temp_surf_int, temp_air_int)
    resistance_air_out = calc_surface_resistance(structure, temp_surf_ext, temp_air_ext)
